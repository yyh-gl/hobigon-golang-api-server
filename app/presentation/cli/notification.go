...
type Notification interface {
	...
	NotifyToLINE(c *cli.Context) error
}
...
func (n notification) NotifyToLINE(c *cli.Context) error {
	ctx := context.TODO()
	ctx = context.WithValue(ctx, app.ContextKeyCLI, c)

	botKey := c.String("bot-key")
	messageKey := c.String("message-key")
	if err := n.u.NotifyToLINE(ctx, botKey, messageKey); err != nil {
		log.Error(ctx, err)
		return err
	}
	return nil
